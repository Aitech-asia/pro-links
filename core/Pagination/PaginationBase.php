...
abstract class PaginationBase
{
    ...
    public function compute()
    {
        ...
        if ($page_count == 1) {
            return;
        }		// there are no more pages than one, paginaiton is useless
        ...
        if ($ending_range > ($page_count-1)) {
            $ending_range = ($page_count - 1);
        }
        ...
        for ($i = $starting_range; $i <= $ending_range; $i++) {
            ...
            // ## ENDING 3 DOTS ( ... )
            elseif ($i == $ending_range && ($ending_range != ($page_count - 1))) {
